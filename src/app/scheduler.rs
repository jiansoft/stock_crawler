...
use crate::{app::backfill::{
    ...
    }, app::event, core::declare, interfaces::bot::{self, telegram::Telegram}};
...
pub async fn start(sched: &JobScheduler) -> Result<()> {
    ...
    tracing::info!("scheduler start done: run_cron elapsed={:?}",
    ...
    if declare::StockExchange::TWSE.is_open() {
        ...
        tracing::info!("scheduler start done: opening trace::stock_price elapsed={:?}",
    ...
    tracing::info!("scheduler start done: telegram notify elapsed={:?}",
        telegram_timer.elapsed());
    tracing::info!("scheduler start done: total elapsed={:?}",
        timer.elapsed());
...
async fn run_cron(sched: &JobScheduler) -> Result<()> {
    ...
    let jobs = vec![
        // 01:00 更新興櫃股票的每股淨值
        create_job("0 0 1 * * *", net_asset_value_per_share::emerging::execute),
        // 02:30 更新盈餘分配率
        create_job("0 30 2 * * *", dividend::payout_ratio::execute),
        // 03:00 更新台股季度財報
        create_job("0 0 3 * * *", event::taiwan_stock::quarter_eps::execute),
        // 04:00 更新台股季度財報(ROE、ROA為零的數據)
        create_job("0 0 4 * * *", financial_statement::quarter::execute),
        // 05:00 更新台股年度財報(僅有eps 等少數欄位的資料)
        create_job("0 0 5 * * *", event::taiwan_stock::annual_eps::execute),
        // 05:05 更新台股年度財報
        create_job("0 5 5 * * *", financial_statement::annual::execute),
        ...
        // 05:15 取得台股的營收
        create_job("0 15 5 * * *", revenue::execute),
        // 05:20 更新台股國際證券識別碼
        create_job("0 20 5 * * *", isin::execute),
        // 05:25 更新下市的股票
        ...
        // 08:04 提醒本日開始公開申購的股票
        create_job("0 4 8 * * *", event::taiwan_stock::public::execute),
        // 09:00 更新股票權值佔比
        create_job("0 0 9 * * *", stock_weight::execute),
        // 09:02 提醒本日已達高低標的股票有那些
        create_job("0 2 9 * * *", event::trace::stock_price::execute),
        // 15:00 取得收盤報價數據
        create_job("0 0 15 * * *", event::taiwan_stock::closing::execute),
        // 21:00 資料庫內尚未有年度配息數據的股票取出後向第三方查詢後更新回資料庫
        create_job("0 0 21 * * *", dividend::execute),
        // 22:00 外資持股狀態
        create_job(
            "0 0 22 * * *",
    ...
    tracing::info!("scheduler run_cron done: jobs={}, elapsed={:?}",
        job_count,
        timer.elapsed());
