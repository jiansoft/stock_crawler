...
pub async fn fetch_last_two_month() -> Result<Vec<Entity>, Error> {
    ...
    let last_month_int =
        (last_month_timezone.year() * 100) + last_month_timezone.month() as i32;
...
order by "Serial" desc
        ...
        .try_map(|row: PgRow| {
            ...
            let compared_with_last_year_same_month =
                row.try_get("ComparedWithLastYearSameMonth")?;
            let accumulated_compared_with_last_year =
                row.try_get("AccumulatedComparedWithLastYear")?;
...
mod tests {
    ...
    use crate::internal::database::model::revenue::{fetch_last_two_month};
    use crate::logging;
