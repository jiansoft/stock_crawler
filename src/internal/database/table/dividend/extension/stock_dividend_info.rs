...
/// 取得指定日期為股利發放日的股票
pub async fn fetch_stocks_with_payable_on_date(
    date: NaiveDate,
) -> Result<Vec<StockDividendInfo>> {
    let sql = r#"
SELECT
...
mod tests {
    ...
    async fn test_fetch_stocks_with_payable_on_date() {
        ...
        let d = ex_date.date_naive();
        match fetch_stocks_with_payable_on_date(d).await {
            Ok(cd) => {
                dbg!(&cd);
                logging::debug_file_async(format!("stock: {:?}", cd));
            }
            Err(why) => {
                logging::debug_file_async(format!("Failed to fetch_stocks_with_payable_on_date because {:?}", why));
            }
        }
