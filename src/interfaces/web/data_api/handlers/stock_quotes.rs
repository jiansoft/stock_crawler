//! `/stocks/*` 的報價與基本資料 handlers（搜尋、最新報價、歷史日線、
//! 完整基本面與近即時快照）及其對應的資料庫列型別。
...
use super::{database_error, decimal_to_f64, error_response, parse_range, timestamp};
use crate::infra::{cache::SHARE, database};
use crate::interfaces::web::data_api::dto::{
    DailyQuote, ErrorBody, HistoricalQuote, HistoryParams, LatestQuoteResponse,
...
pub(crate) async fn search_stocks(Query(params): Query<SearchParams>) -> Response {
...
pub(crate) async fn latest_quote(Path(symbol): Path<String>) -> Response {
...
pub(crate) async fn stock_profile(Path(symbol): Path<String>) -> Response {
...
pub(crate) async fn realtime_snapshot(Path(symbol): Path<String>) -> Response {
