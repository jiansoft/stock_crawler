...
//!
//! 這個模組把 gRPC manual backfill API 轉接到 `web::manual_backfill` 的共用
...
/// Manual backfill gRPC service implementation.
///
/// 服務本身不保存狀態；所有 job 狀態都委派給 `web::manual_backfill` 的全域
...
impl ManualBackfill for ManualBackfillService {
    ...
    async fn start_closing_aggregate(
        ...
        // 建立共用背景 job，讓 HTTP UI 也能查到這筆 gRPC 建立的工作。
        let job = web::manual_backfill::start_closing_aggregate_job(date).await;
        // 轉成 proto response model 後回傳。
        Ok(Response::new(BackfillJobResponse {
            job: Some(to_grpc_job(job)),
        }))
    ...
    async fn start_received_dividend_records(
        ...
        let security_code =
            web::manual_backfill::normalize_security_code(req.into_inner().security_code)
                .map_err(|why| Status::invalid_argument(why.to_string()))?;

        // 建立背景 job，立即回傳 job id 與初始狀態。
        let job = web::manual_backfill::start_received_dividend_records_job(security_code).await;
        Ok(Response::new(BackfillJobResponse {
            job: Some(to_grpc_job(job)),
        }))
    }
    ...
    async fn list_jobs(
        ...
        let jobs = web::manual_backfill::list_backfill_jobs()
            .await
            .into_iter()
            .map(to_grpc_job)
            .collect();
    ...
    async fn get_job(
        ...
        let job = web::manual_backfill::get_backfill_job(&id)
            .await
...
fn to_grpc_job(job: web::manual_backfill::BackfillJob) -> BackfillJob {
