...
pub fn start_caching_task() {
    ...
    let handle = tokio::spawn(async move {
        ...
        tracing::info!("Yahoo 類股快取任務啟動 generation={} active_tasks={}",
        ...
        while IS_CACHING.load(Ordering::SeqCst) {
            ...
            for category in &categories {
                ...
                let sleep_duration = if is_denied {
                    tracing::warn!("Yahoo 採集遭遇 Request denied，將強制冷卻等待 10 分鐘。類股: {} {}({})",
                        category.exchange.label(),
                        category.name,
                        category.sector_id);
            ...
            if total_count == 0 {
                tracing::error!("Yahoo 類股快取輪詢完成但沒有任何資料落地: success_count={} failure_count={} 耗時 {:?}",
                    success_count,
                    failure_count,
                    cycle_started.elapsed());
            }

            tracing::debug!("Yahoo 類股快取輪詢完成，共 {} 檔股票，成功類股 {}，失敗類股 {}，candidate_events={}，耗時 {:?}",
                total_count,
                success_count,
                failure_count,
                candidate_event_count,
        ...
        tracing::info!("Yahoo 類股快取任務已停止 generation={} active_tasks={}",
            generation, active_tasks);
    });
    ...
    if let Ok(mut task) = CACHING_TASK.lock() {
        *task = Some(handle);
        ...
        tracing::error!("{}", "Failed to store Yahoo caching task handle".to_string(),);
    }
}
...
pub async fn stop_caching_task() {
    ...
    let handle = match CACHING_TASK.lock() {
        ...
        Err(why) => {
            tracing::error!("Failed to lock Yahoo caching task handle because {:?}",
                why);
            None
        }
    };
    ...
    {
        tracing::error!("Yahoo 類股快取任務停止等待失敗: {:?}",
            why);
    }
...
fn apply_category_snapshots(
    ...
    match SHARE.stock_snapshots.write() {
        ...
        Err(why) => {
            ...
            tracing::error!("Failed to update Yahoo 類股快取 because {:?}",
                why);
