...
pub fn start_price_tasks() {
    ...
    crate::infra::crawler::histock::price::start_caching_task();
    // 再啟動 Yahoo 類股輪詢任務，讓兩個來源都能持續提供即時資料。
    crate::infra::crawler::yahoo::price::start_caching_task();
}
...
pub async fn stop_price_tasks() {
    ...
    crate::infra::crawler::histock::price::stop_caching_task().await;
    // 再停 Yahoo，避免收盤後仍有類股輪詢在背景更新快取。
    crate::infra::crawler::yahoo::price::stop_caching_task().await;
}
